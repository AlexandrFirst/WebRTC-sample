...
connection.onmessage = (msg) => {
    ...
    switch (data.type) {
        case "login":
            ...
            break;
        case "offer":
            call_status.innerHTML = '<div class="calling-status-wrap card black white-text"> <div class="user-image"> <img src="assets/images/me.jpg" class="caller-image circle" alt=""> </div> <div class="user-name">' + data.name + '</div> <div class="user-calling-status">Calling...</div> <div class="calling-action"> <div class="call-accept"><i class="material-icons green darken-2 white-text audio-icon">call</i></div> <div class="call-rejet"><i class="material-icons red darken-3 white-text close-icon">close</i></div> </div> </div>'

            var call_receive = document.querySelector('.call-accept');
            var call_reject = document.querySelector('.call-rejet');
            ...
            call_receive.addEventListener('click', function () {
                ...
                audio_toggle.onclick = function () {
                    ...
                    var audioo_toggle_class = document.querySelector(".audio-toggle");
                    if (audioo_toggle_class.innerText == 'mic') {
                        audioo_toggle_class.innerText = 'mic_off';
                    } else {
                        audioo_toggle_class.innerText = 'mic';
                    }
                }
...
call_btn.addEventListener("click", () => {
    ...
    call_status.innerHTML = '<div class="calling-status-wrap card black white-text"> <div class="user-image"> <img src="assets/images/other.jpg" class="caller-image circle" alt=""> </div> <div class="user-name">' + call_to_username + '</div> <div class="user-calling-status">Calling...</div> <div class="calling-action"> <div class="call-rejet"><i class="material-icons red darken-3 white-text close-icon">close</i></div> </div> </div>'
    ...
    if (call_to_username.length > 0) {
        ...
        myConn.createOffer(function (offer) {
            ...
            console.log("local sdp from offer" + JSON.stringify(offer));
    ...
    if (success === false) {
        ...
        navigator.getUserMedia({
            ...
            myConn.ondatachannel = function (event) {
                let myDataChannel = event.channel;
                myDataChannel.onmessage = messageReceiveCallback;
                myDataChannel.onerror = errorCallback;
                myDataChannel.onclose = closeCallback;
            ...
            dataChannel.onmessage = messageReceiveCallback;

            dataChannel.onclose = closeCallback;
...
function messageReceiveCallback(event) {
...
function closeCallback() {
    console.log("data channels is closed");
}

function offerProcess(offer, name) {
    connected_user = name;
    console.log("remote sdp from offer" + JSON.stringify(offer));
    myConn.setRemoteDescription(new RTCSessionDescription(offer));

    //alert(name);

    myConn.createAnswer(function (answer) {
        console.log("local sdp from answer" + JSON.stringify(answer));
    ...
    //create answear to the requesting user
}

function answerProcess(answer) {
    console.log("remote sdp from anwer" + JSON.stringify(answer));
